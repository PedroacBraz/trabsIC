...
void cyclictxCrossover(int[12], int[12], int[][12], int, int);
void pmxtxCrossover(int[12], int[12], int[][12], int , int );
void pmx(int[12], int[12], int[][12], int );
void mutation(int[][12]);
void procreate(int[SIZE_POPULATION][12],int[][12]);
void statistics(TStatistics*, int[][12], int[12], int);
void finilizeStatistics(TStatistics* );
void quicksort(int[][12], int, int);
void ordenedReinsertion(int[][12], int[][12], int[][12]);
void condense(int[][12], int[][12], int[][12]);
int main(int argc, const char * argv[]) {
    // insert code here...
    srand(time(NULL));
    TStatistics stats;
    int i,j, bestSubject[11];
    int possibilities[CHOICE_MAX]; // numeros de 0 a 9
    int population[SIZE_POPULATION][12]; //100 individuos com S=0, E=1, N=2, D=3, M=4, O=5, R=6, Y=7 ,8 e 9 mantidos para o funcionamento do txCrossover ciclico e index 10 reservado para a fitness
    int children[txCrossover][12]; //mesmo formato da population tirando os tickets da roleta
    ...
    for(i=0;i< REPEAT; i++){
        ...
        for(j=0; j< GENERATIONS; j++){
            ...
            quicksort(children, 0, txCrossover);
            condense(population, children, newPopulation);
...
int roulete(int parents[SIZE_POPULATION][12]){ //ta configurado para maximacao de fitness. tem que configurar para minimizar fitness
...
void cyclictxCrossover(int parent1[12], int parent2[12], int children[][12], int idxChild1, int idxChild2){
    int i, j, first, next;
    for(i=0; i<10; i++){
...
void pmxtxCrossover(int parent1[12], int parent2[12], int children[][12], int idxChild1, int idxChild2){
...
void mutation(int children[][12]){
    int lucky[txMutation][3], i;
    for(i=0; i< txMutation; i++){
        lucky[i][0] = rand()%txCrossover;
        do{
            lucky[i][1] = rand()%8;
            lucky[i][2] = rand()%8;
...
void procreate(int parents[SIZE_POPULATION][12],int children[txCrossover][12]){
    int i;
    //printf("Filhos gerados: \n\n");
    for(i = 0;i < (txCrossover/2); i++){
        int parent1 = tournament(parents), parent2 = tournament(parents);
        cyclictxCrossover(parents[parent1], parents[parent2], children, i, i+txCrossover/2);
    }
    //sorteando mutacao
    mutation(children);
}
...
void ordenedReinsertion(int parents[][12], int children[][12], int condensedPopulation[][12]){
    int i, j, k;
    i = j = k = 0;
...
void condense(int parents[][12], int children[][12], int condensedPopulation[][12]){
    ordenedReinsertion(parents,children,condensedPopulation);
}
